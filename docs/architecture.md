...
| Live-control application | `src/mcp/service.ts`, `reconcile.ts`, `state-store.ts`, `edit-history-store.ts`, `patch-history-identity.ts` | Coordinate desired state, atomic recovery state, append-only edit evidence, and explicit history identity decisions |
